...
class lime_output_color extends lime_output
{
  ...
  {
    $message = preg_replace_callback('/(?:^|\.)((?:not ok|dubious) *\d*)\b/',
	function($matches) {
             return $this->colorizer->colorize($matches[0], 'ERROR'); 
	},
	$message
    );
    $message = preg_replace_callback('/(?:^|\.)(ok *\d*)\b/',
	function($matches) {
             return $this->colorizer->colorize($matches[0], 'INFO');
	},
        $message
    );
    $message = preg_replace_callback('/"(.+?)"/',
	function($matches) {
             return $this->colorizer->colorize($matches[0], 'PARAMETER');
        },
	$message
    );
    $message = preg_replace_callback('/(\->|\:\:)?([a-zA-Z0-9_]+?)\(\)/',
	function($matches) {
